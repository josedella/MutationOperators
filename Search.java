	...
	//	Initialize RNG, array sizes and other objects
		...
		// *********************************************************************
				...
				for (int i=0; i<Parameters.popSize; i=i+2) {
					...
					//	Select Two Parents
					parent1 = Chromo.selectParent(100);
					parent2 = parent1;
					while (parent2 == parent1) {
						parent2 = Chromo.selectParent(100);
					}
		...
		summaryOutput.write("Gen                 AvgFit              BestFit              Simpson's Diversity Index \n");
		for (int i=0; i<Parameters.generations; i++) {
			...
			Hwrite.left(fitnessStats[2][i]/Parameters.numRuns, 20, 4, summaryOutput);
			summaryOutput.write("\n");
		}
		summaryOutput.write(bestOverAllChromo.chromo);

		// for(int i=0; i<Parameters.generations; i++) {
		// 	double genAverage = fitnessStats[0][i]/Parameters.numRuns;
		// 	double genBestAverage = fitnessStats[1][i]/Parameters.numRuns;

		// 	double averageAverage = averageAverage + genAverage;
		// 	double bestAverage = bestAverage + genBestAverage;
		// }

		// sumRawFitness2 = sumRawFitness2 +
		// 				member[i].rawFitness * member[i].rawFitness;
		// stddv = Math.sqrt(Math.abs(sumRawFitness2 - sumRawFitness*sumRawFitness/Parameters.popSize)/(Parameters.popSize-1));
